export default {
    map: {
        unfocused: "Unfocused - click here to focus",
    },
    modes: {
        sprint_10l: {
            title: "Sprint",
            subtitle: "10L",
            description: "Clear 10 lines!"
        },
        sprint_20l: {
            title: "Sprint",
            subtitle: "20L",
            description: "Clear 20 lines!"
        },
        sprint_40l: {
            title: "Sprint",
            subtitle: "40L",
            description: "Clear 40 lines!"
        },
        sprint_100l: {
            title: "Sprint",
            subtitle: "100L",
            description: "Clear 100 lines!"
        },
        sprint_400l: {
            title: "Sprint",
            subtitle: "400L",
            description: "Clear 400 lines!"
        },
        sprint_1000l: {
            title: "Sprint",
            subtitle: "1000L",
            description: "Clear 1000 lines!"
        },
        sprintPenta: {
            title: "Sprint",
            subtitle: "PENTOMINO",
            description: "40L with 18 pentominoes"
        },
        sprintMPH: {
            title: "Sprint",
            subtitle: "MPH",
            description: "Memoryless\nPreviewless\nHoldless"
        },
        sprint123: {
            title: "Sprint",
            subtitle: "M123",
            description: "40L with only monominoes, dominoes, and triminoes"
        },
        construct_sg: {
            title: "Construct",
            subtitle: "SECRET GRADE",
            description: "Build a zigzag pattern by following the guide!"
        },
        construct_checker: {
            title: "Construct",
            subtitle: "CHECKERBOARD",
            description: "Build a checkerboard pattern!"
        },
        construct_invsg: {
            title: "Construct",
            subtitle: "INV. SG",
            description: "Build an inverted zigzag pattern!"
        },
        dig_10l: {
            title: "Dig",
            subtitle: "10L",
            description: "Dig 10 garbage lines as fast as you can!"
        },
        dig_40l: {
            title: "Dig",
            subtitle: "40L",
            description: "Dig 40 garbage lines as fast as you can!"
        },
        dig_100l: {
            title: "Dig",
            subtitle: "100L",
            description: "Dig 100 garbage lines as fast as you can!"
        },
        dig_400l: {
            title: "Dig",
            subtitle: "400L",
            description: "Dig 400 garbage lines as fast as you can!"
        },
        dig_eff_10l: {
            title: "Dig",
            subtitle: "EFFICIENCY 10L",
            description: "Dig 10 garbage lines with the least pieces!"
        },
        dig_eff_40l: {
            title: "Dig",
            subtitle: "EFFICIENCY 40L",
            description: "Dig 40 garbage lines with the least pieces!"
        },
        dig_eff_100l: {
            title: "Dig",
            subtitle: "EFFICIENCY 100L",
            description: "Dig 100 garbage lines with the least pieces!"
        },
        dig_eff_400l: {
            title: "Dig",
            subtitle: "EFFICIENCY 400L",
            description: "Dig 400 garbage lines with the least pieces!"
        },
        dig_quad_10l: {
            title: "Dig",
            subtitle: "TECHRASH 10L",
            description: "Dig 10 garbage lines using only techrash!"
        },
        drought_n: {
            title: "Drought",
            subtitle: "100L",
            description: "100L but without I pieces"
        },
        drought_l: {
            title: "Drought+",
            subtitle: "100L",
            description: "100L but the randomizer hates you"
        },
        marathon_n: {
            title: "Marathon",
            subtitle: "NORMAL",
            description: "200-line marathon with increasing speed"
        },
        marathon_h: {
            title: "Marathon",
            subtitle: "HARD",
            description: "200-line high-speed marathon"
        },
        solo_e: {
            title: "Battle",
            subtitle: "EASY",
            description: "Defeat the AI!"
        },
        solo_n: {
            title: "Battle",
            subtitle: "NORMAL",
            description: "Defeat the AI!"
        },
        solo_h: {
            title: "Battle",
            subtitle: "HARD",
            description: "Defeat the AI!"
        },
        solo_l: {
            title: "Battle",
            subtitle: "LUNATIC",
            description: "Defeat the AI!"
        },
        solo_u: {
            title: "Battle",
            subtitle: "ULTIMATE",
            description: "Defeat the AI!"
        },
        techmino49_e: {
            title: "Tech VS 49",
            subtitle: "EASY",
            description: "49-player battle.\nThe last one standing wins!"
        },
        techmino49_h: {
            title: "Tech VS 49",
            subtitle: "HARD",
            description: "49-player battle.\nThe last one standing wins!"
        },
        techmino49_u: {
            title: "Tech VS 49",
            subtitle: "ULTIMATE",
            description: "49-player battle.\nThe last one standing wins!"
        },
        techmino99_e: {
            title: "Tech VS 99",
            subtitle: "EASY",
            description: "99-player battle.\nThe last one standing wins!"
        },
        techmino99_h: {
            title: "Tech VS 99",
            subtitle: "HARD",
            description: "99-player battle.\nThe last one standing wins!"
        },
        techmino99_u: {
            title: "Tech VS 99",
            subtitle: "ULTIMATE",
            description: "99-player battle.\nThe last one standing wins!"
        },
        round_e: {
            title: "Turn-Based",
            subtitle: "EASY",
            description: "Take turns to play against the AI!"
        },
        round_n: {
            title: "Turn-Based",
            subtitle: "NORMAL",
            description: "Take turns to play against the AI!"
        },
        round_h: {
            title: "Turn-Based",
            subtitle: "HARD",
            description: "Take turns to play against the AI!"
        },
        round_l: {
            title: "Turn-Based",
            subtitle: "LUNATIC",
            description: "Take turns to play against the AI!"
        },
        round_u: {
            title: "Turn-Based",
            subtitle: "ULTIMATE",
            description: "Take turns to play against the AI!"
        },
        big_n: {
            title: "Big",
            subtitle: "NORMAL",
            description: "Play in a 5×10 field!"
        },
        big_h: {
            title: "Big",
            subtitle: "HARD",
            description: "Play in a 5×10 field!"
        },
        master_n: {
            title: "Master",
            subtitle: "NORMAL",
            description: "For 20G beginners"
        },
        master_h: {
            title: "Master",
            subtitle: "HARD",
            description: "For 20G pros"
        },
        master_m: {
            title: "Master",
            subtitle: "M21",
            description: "For 20G masters"
        },
        master_final: {
            title: "Master",
            subtitle: "FINAL",
            description: "20G and beyond"
        },
        master_ph: {
            title: "Master",
            subtitle: "PHANTASM",
            description: "???"
        },
        master_g: {
            title: "Master",
            subtitle: "GRADED",
            description: "Get the highest grade you can!"
        },
        master_ex: {
            title: "GrandMaster",
            subtitle: "EXTRA",
            description: "An eternity shorter than an instant" // i have no idea what this means, but it's what the game says
        },
        master_instinct: {
            title: "Master",
            subtitle: "INSTINCT",
            description: "What if the active piece becomes invisible?"
        },
        strategy_e: {
            title: "Strategy",
            subtitle: "EASY",
            description: "Fast 20G decisions"
        },
        strategy_h: {
            title: "Strategy",
            subtitle: "HARD",
            description: "Fast 20G decisions"
        },
        strategy_u: {
            title: "Strategy",
            subtitle: "ULTIMATE",
            description: "Fast 20G decisions"
        },
        strategy_e_plus: {
            title: "Strategy",
            subtitle: "EASY+",
            description: "Holdless Strategy!"
        },
        strategy_h_plus: {
            title: "Strategy",
            subtitle: "HARD+",
            description: "Holdless Strategy!"
        },
        strategy_u_plus: {
            title: "Strategy",
            subtitle: "ULTIMATE+",
            description: "Holdless Strategy!"
        },
        blind_e: {
            title: "Invisible",
            subtitle: "SLOW",
            description: "For beginners"
        },
        blind_n: {
            title: "Invisible",
            subtitle: "FAST",
            description: "For intermediates"
        },
        blind_h: {
            title: "Invisible",
            subtitle: "INSTANT",
            description: "For the experienced"
        },
        blind_l: {
            title: "Invisible",
            subtitle: "NO GHOST",
            description: "For professionals"
        },
        blind_u: {
            title: "Invisible",
            subtitle: "NO FIELD",
            description: "Are you ready?"
        },
        blind_wtf: {
            title: "Invisible",
            subtitle: "VOID",
            description: "You're not ready."
        },
        classic_e: {
            title: "Classic",
            subtitle: "EASY",
            description: "A low-speed recreation of the game from the 80s"
        },
        classic_n: {
            title: "Classic",
            subtitle: "NORMAL",
            description: "A medium-speed recreation of the game from the 80s"
        },
        classic_h: {
            title: "Classic",
            subtitle: "HARD",
            description: "A medium-speed recreation of the game from the 80s"
        },
        classic_l: {
            title: "Classic",
            subtitle: "LUNATIC",
            description: "A high-speed recreation of the game from the 80s"
        },
        classic_u: {
            title: "Classic",
            subtitle: "ULTIMATE",
            description: "An extreme-speed recreation of the game from the 80s"
        },
        survivor_e: {
            title: "Survival",
            subtitle: "EASY",
            description: "How long can you survive?"
        },
        survivor_n: {
            title: "Survival",
            subtitle: "NORMAL",
            description: "How long can you survive?"
        },
        survivor_h: {
            title: "Survival",
            subtitle: "HARD",
            description: "How long can you survive?"
        },
        survivor_l: {
            title: "Survival",
            subtitle: "LUNATIC",
            description: "How long can you survive?"
        },
        survivor_u: {
            title: "Survival",
            subtitle: "ULTIMATE",
            description: "How long can you survive?"
        },
        attacker_h: {
            title: "Attacker",
            subtitle: "HARD",
            description: "Practice your attacking skills!"
        },
        attacker_u: {
            title: "Attacker",
            subtitle: "ULTIMATE",
            description: "Practice your attacking skills!"
        },
        defender_n: {
            title: "Defender",
            subtitle: "NORMAL",
            description: "Practice your defending skills!"
        },
        defender_l: {
            title: "Defender",
            subtitle: "LUNATIC",
            description: "Practice your defending skills!"
        },
        dig_h: {
            title: "Driller",
            subtitle: "HARD",
            description: "Digging practice!"
        },
        dig_u: {
            title: "Driller",
            subtitle: "ULTIMATE",
            description: "Digging practice!"
        },
        c4wtrain_n: {
            title: "C4W Training",
            subtitle: "NORMAL",
            description: "Infinite combos"
        },
        c4wtrain_l: {
            title: "C4W Training",
            subtitle: "LUNATIC",
            description: "Infinite combos"
        },
        pctrain_n: {
            title: "PC Training",
            subtitle: "NORMAL",
            description: "Perfect Clear practice"
        },
        pctrain_l: {
            title: "PC Training",
            subtitle: "LUNATIC",
            description: "A harder Perfect Clear practice"
        },
        pc_n: {
            title: "PC Challenge",
            subtitle: "NORMAL",
            description: "Get PCs within 100 lines!"
        },
        pc_h: {
            title: "PC Challenge",
            subtitle: "HARD",
            description: "Get PCs within 100 lines!"
        },
        pc_l: {
            title: "PC Challenge",
            subtitle: "LUNATIC",
            description: "Get PCs within 100 lines!"
        },
        pc_inf: {
            title: "PC Challenge",
            subtitle: "INFINITE",
            description: "Get as many PCs as you can!"
        },
        tech_n: {
            title: "Tech B2B",
            subtitle: "NORMAL",
            description: "Try to keep the Back-to-Back chain!"
        },
        tech_n_plus: {
            title: "Tech B2B",
            subtitle: "NORMAL+",
            description: "Spins & PCs only"
        },
        tech_h: {
            title: "Tech B2B",
            subtitle: "HARD",
            description: "Try to keep the Back-to-Back chain!"
        },
        tech_h_plus: {
            title: "Tech B2B",
            subtitle: "HARD+",
            description: "Spins & PCs only"
        },
        tech_l: {
            title: "Tech B2B",
            subtitle: "LUNATIC",
            description: "Try to keep the Back-to-Back chain!"
        },
        tech_l_plus: {
            title: "Tech B2B",
            subtitle: "LUNATIC+",
            description: "Spins & PCs only"
        },
        tech_finesse: {
            title: "Tech Finesse",
            subtitle: "",
            description: "No finesse errors allowed!"
        },
        tech_finesse_f: { // idk why it's tech_finesse_f instead of tech_finesse_plus but whatever
            title: "Tech Finesse",
            subtitle: "PLUS",
            description: "No normal clears nor finesse errors allowed!"
        },
        tsd_e: {
            title: "TSD Challenge",
            subtitle: "EASY",
            description: "T-Spin Doubles only!"
        },
        tsd_h: {
            title: "TSD Challenge",
            subtitle: "HARD",
            description: "T-Spin Doubles only!"
        },
        tsd_u: {
            title: "TSD Challenge",
            subtitle: "ULTIMATE",
            description: "T-Spin Doubles only!"
        },
        backfire_n: {
            title: "Backfire",
            subtitle: "NORMAL",
            description: "Hold back the backfiring garbage lines!"
        },
        backfire_h: {
            title: "Backfire",
            subtitle: "HARD",
            description: "Hold back the backfiring garbage lines!"
        },
        backfire_l: {
            title: "Backfire",
            subtitle: "LUNATIC",
            description: "Hold back the backfiring garbage lines!"
        },
        backfire_u: {
            title: "Backfire",
            subtitle: "ULTIMATE",
            description: "Hold back the backfiring garbage lines!"
        },
        sprintAtk: {
            title: "Sprint",
            subtitle: "100 ATTACK",
            description: "Send 100 lines!"
        },
        sprintEff: {
            title: "Sprint",
            subtitle: "EFFICIENCY",
            description: "Send as much attack as you can in 40 lines!"
        },
        zen: {
            title: "Zen",
            subtitle: "200L",
            description: "A 200-line run with no time limit"
        },
        ultra: {
            title: "Ultra",
            subtitle: "EXTRA",
            description: "A 2-minute score attack"            
        },
        infinite: {
            title: "Infinite",
            subtitle: "",
            description: "Just a sandbox"
        },
        infinite_dig: {
            title: "Infinite",
            subtitle: "DIG",
            description: "Dig-diggin'-dug" // sounds kinda cringe but whatever xd
        },
        marathon_inf: {
            title: "Marathon",
            subtitle: "INFINITE",
            description: "Infinite marathon." // sike it ends after 1700 lines (but it's not humanly possible to reach that far)
        },
        
        // Extra modes
        sprintFix: {
            title: "Sprint",
            subtitle: "NO LEFT/RIGHT",
            description: "40L with no left/right movement"
        },
        sprintLock: {
            title: "Sprint",
            subtitle: "NO ROTATIONS",
            description: "40L with no rotations"
        },
        sprintSmooth: {
            title: "Sprint",
            subtitle: "NO FRICTION",
            description: "40L with zero DAS and ARR"
        },
        marathon_bfmax: {
            title: "Marathon",
            subtitle: "ULTIMATE",
            description: "200-line marathon, but you can only do non-combo singles"
        },
        clear_rush: {
            title: "Clear Rush",
            subtitle: "NORMAL",
            description: "All-spin tutorial!\n(scrapped mode)",
            version_info: "v0.17.0 - v0.17.3"
        },
        rhythm_e: {
            title: "Rhythm",
            subtitle: "EASY",
            description: "200-line low-tempo rhythmic marathon",
            version_info: "v0.13.2 - v0.16.4"
        },
        rhythm_h: {
            title: "Rhythm",
            subtitle: "HARD",
            description: "200-line medium-tempo rhythmic marathon",
            version_info: "v0.13.2 - v0.16.4"
        },
        rhythm_u: {
            title: "Rhythm",
            subtitle: "ULTIMATE",
            description: "200-line high-tempo rhythmic marathon",
            version_info: "v0.13.2 - v0.16.4"
        },

        // Outdated modes
        // https://gist.github.com/Not-A-Normal-Robot/5d08da1ad9db07ba590fa67a47898668
        master_l: { // renamed to master_n in v0.16.0
            title: "Master",
            subtitle: "LUNATIC",
            description: "For 20G beginners"
        },
        master_u: { // renamed to master_h in v0.16.0
            title: "Master",
            subtitle: "ULTIMATE",
            description: "For 20G pros"
        },
        classic_fast: {
            title: "Classic",
            subtitle: "CTWC",
            description: "High-speed classic mode."
        },
        bigbang: {
            title: "Big Bang",
            subtitle: "EASY",
            description: "All-spin tutorial!\n(scrapped mode)",
            version_info: "v0.8.5 - v0.17.0"
        },
        tech_finesse_lock: {
            title: "Tech Finesse",
            subtitle: "LOCK",
            description: "No finesse errors, combined with limited inputs!",
            version_info: "9769b33 - d3e4d7e"
        },
        tech_finesse_lock_plus: {
            title: "Tech Finesse",
            subtitle: "LOCK PLUS",
            description: "No normal clears nor finesse errors combined with limited inputs!",
            version_info: "9769b33 - d3e4d7e"
        },
        stack_v1_20l: {
            title: "Stack (v1)",
            subtitle: "20L",
            description: "Pack them!",
            version_info: "v0.16.0"
        },
        stack_v1_40l: {
            title: "Stack (v1)",
            subtitle: "40L",
            description: "Pack them!",
            version_info: "v0.16.0"
        },
        stack_v1_100l: {
            title: "Stack (v1)",
            subtitle: "100L",
            description: "Pack them!",
            version_info: "v0.16.0"
        },
        stack_v2_e: {
            title: "Stack (v2)",
            subtitle: "EASY",
            description: "Pack them!",
            version_info: "v0.16.1 - v0.16.2"
        },
        stack_v2_h: {
            title: "Stack (v2)",
            subtitle: "HARD",
            description: "Pack them!",
            version_info: "v0.16.1 - v0.16.2"
        },
        stack_v2_u: {
            title: "Stack (v2)",
            subtitle: "ULTIMATE",
            description: "Pack them!",
            version_info: "v0.16.1 - v0.16.2"
        },
        stack_v3_e: {
            title: "Stack (v3)",
            subtitle: "EASY",
            description: "Pack them!",
            version_info: "v0.16.4"
        },
        stack_v3_h: {
            title: "Stack (v3)",
            subtitle: "HARD",
            description: "Pack them!",
            version_info: "v0.16.4"
        },
        stack_v3_u: {
            title: "Stack (v3)",
            subtitle: "ULTIMATE",
            description: "Pack them!",
            version_info: "v0.16.4"
        },
        stack_v4_e: {
            title: "Stack (v4)",
            subtitle: "EASY",
            description: "Pack them!",
            version_info: "v0.16.5 - v0.17.4"
        },
        stack_v4_u: {
            title: "Stack (v4)",
            subtitle: "ULTIMATE",
            description: "Pack them!",
            version_info: "v0.16.5 - v0.17.4"
        },
        stack_e: {
            title: "Stack (v5)",
            subtitle: "EASY",
            description: "Pack them!",
            version_info: "v0.17.5+"
        },
        stack_u: {
            title: "Stack (v5)",
            subtitle: "ULTIMATE",
            description: "Pack them!",
            version_info: "v0.17.5+"
        },

        // Techmino UE modes
        // https://github.com/Another-Soul/Techmino-Unofficial-Expansion
        master_death: {
            title: "Master",
            subtitle: "DEATH",
            description: "Fast speed mode! Can you break the lv500 barrier?",
            version_info: "Unofficial Expansion"
        },
        master_phantom: { // not to be confused with Master PHANTASM
            title: "Master",
            subtitle: "PHANTOM",
            description: "The locked blocks are invisible. What will you do?",
            version_info: "Unofficial Expansion"
        },

        // Unused modes
        custom_clear: {
            title: "Custom",
            subtitle: "NORMAL",
            description: ""
        },
        custom_puzzle: {
            title: "Custom",
            subtitle: "PUZZLE",
            description: ""
        },
        netBattle: {
            title: "Battle",
            subtitle: "ONLINE",
            description: ""
        },
        hotseat_2p: {
            title: "Battle",
            subtitle: "LOCAL 2P",
            description: "",
            version_info: "v0.8.4(?) - v0.8.18"
        },
        hotseat_3p: {
            title: "Battle",
            subtitle: "LOCAL 3P",
            description: "",
            version_info: "v0.8.4(?) - v0.8.18"
        },
        hotseat_4p: {
            title: "Battle",
            subtitle: "LOCAL 4P",
            description: "",
            version_info: "v0.8.4(?) - v0.8.18"
        },
    }
}